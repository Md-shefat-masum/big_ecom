                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $item->email }}</td>
                                        {{-- <td>{{ $item->role_id }}</td> --}}
                                        <td>{{ $item->role_information ? $item->role_information->name : $item->role_id }}
                                        </td>
                                        <td>
                                            <div>
                                                ...
                                                {{-- <a type="button" href="" class="btn btn-success waves-effect waves-light m-1">
                                                            <i class="fa fa-pencil"></i> <span>Assign</span>
                                                        </a> --}}
                                                <form action="{{route('assign_order')}}" method="post"
                                                    id="assign-order" enctype="multipart/form-data">
                                                    @csrf
                                                    <input type="hidden" name="id" value="{{$item->id}}">
                                                    {{-- <input type="hidden" name="email" value="{{$item->email}}"> --}}
                                                    <a href=""
                                                        onclick="event.preventDefault(); confirm('are you sure you want to assign this order no: {{ $order->id }}??') && document.getElementById('assign-order').submit();"
                                                        class="btn btn-success waves-effect waves-light m-1"><span>Assign</span>
                                                    </a>
                                                </form>
